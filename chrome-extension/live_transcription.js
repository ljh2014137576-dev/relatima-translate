...
// --- glue relay: forwards WLK messages to the local dubbing pipeline ---
let glueSocket = null;
const glueRelayUrl = "ws://127.0.0.1:5100/relay";

function connectGlueRelay() {
  try {
    glueSocket = new WebSocket(glueRelayUrl);
    glueSocket.onopen = () => console.log("[glue] relay connected");
    glueSocket.onclose = () => {
      console.log("[glue] relay disconnected; retrying in 2s");
      setTimeout(connectGlueRelay, 2000);
    };
    glueSocket.onerror = () => {
      try { glueSocket.close(); } catch (e) {}
    };
  } catch (e) {
    console.warn("[glue] relay connect failed", e);
  }
}

function forwardToGlue(data) {
  if (glueSocket && glueSocket.readyState === WebSocket.OPEN) {
    try {
      glueSocket.send(JSON.stringify(data));
    } catch (e) {
      console.warn("[glue] relay send failed", e);
    }
  }
}
...
const defaultWebSocketUrl = `${protocol}://${host}${port ? ":" + port : ""}/asr?target_language=zh`;
...
function setupWebSocket() {
  return new Promise((resolve, reject) => {
    ...
    websocket.onmessage = (event) => {
      ...
      // Forward every WLK message to the local glue relay so the dubbing
      // pipeline can consume the finalized Chinese translations.
      if (data.type !== "config") {
        forwardToGlue(data);
      }
